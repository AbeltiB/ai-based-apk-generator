    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "status": "processing",
                "progress": 45,
                "message": "Generating app architecture...",
                "created_at": "2025-01-01T12:00:00Z",
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "cancelled_at": "2025-01-01T12:01:00Z"
            }
        }
        ...
        logger.info(
            ...
            extra={
                ...
                "websocket_instructions": {
                    "endpoint": f"ws://{http_request.base_url.hostname}:{http_request.base_url.port}/ws/{task_id}",
                    "protocol": "JSON",
...
async def cancel_task(task_id: str) -> CancelTaskResponse:
    ...
    Note: Only tasks in "queued" or "processing" status can be cancelled.
    ...
    with log_context(task_id=task_id, endpoint="/api/v1/task", method="DELETE"):
        ...
        try:
            ...
            # Continue anyway - cancellation is still recorded
...
async def get_system_stats() -> SystemStatsResponse:
    ...
    with log_context(endpoint="/api/v1/stats"):
        ...
        try:
            ...
            # Count active tasks (tasks in processing or queued status)
            active_tasks = 0
            # Note: This is a simplified implementation. In production, you might want
            # to use Redis sets or sorted sets to track active tasks efficiently.
        ...
        try:
            # This would require RabbitMQ management API or queue length tracking
        ...
        # Get uptime from main app (would need to be tracked globally)
